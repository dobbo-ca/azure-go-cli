...
func Assign(ctx context.Context, cmd *cobra.Command, resourceGroup, serverName string, identityIDs []string, noWait bool) error {
	...
	for _, id := range identityIDs {
		identities[id] = &armpostgresqlflexibleservers.UserIdentity{}
	}

	fmt.Printf("Assigning user-assigned identities to '%s'...\n", serverName)
	...
	if err != nil {
		return fmt.Errorf("failed to begin identity assign: %w", err)
	}

	if noWait {
		return output.PrintJSON(cmd, map[string]string{"status": "Identity assign started."})
	}

	resp, err := poller.PollUntilDone(ctx, nil)
	if err != nil {
		return fmt.Errorf("identity assign failed: %w", err)
	}
