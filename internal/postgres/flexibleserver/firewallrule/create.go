...
func Create(ctx context.Context, cmd *cobra.Command, resourceGroup, serverName, ruleName, startIP, endIP string, noWait bool) error {
	...
	fmt.Printf("Creating firewall rule '%s' on server '%s'...\n", ruleName, serverName)
	poller, err := client.BeginCreateOrUpdate(ctx, resourceGroup, serverName, ruleName, rule, nil)
	if err != nil {
		return fmt.Errorf("failed to begin firewall rule create: %w", err)
	}

	if noWait {
		return output.PrintJSON(cmd, map[string]string{"status": "Firewall rule create started."})
	}

	resp, err := poller.PollUntilDone(ctx, nil)
	if err != nil {
		return fmt.Errorf("firewall rule create failed: %w", err)
	}
