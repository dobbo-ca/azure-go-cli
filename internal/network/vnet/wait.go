...
// waitDone reports whether polling should stop. found is whether the virtual
// network currently exists; state is its last-seen provisioning state. The
...
func Wait(ctx context.Context, cmd *cobra.Command, name, resourceGroup string, deleted, exists bool, interval, timeout int) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timed out waiting for virtual network '%s' after %d seconds", name, timeout)
		}
