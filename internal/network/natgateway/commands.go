...
func NewNatGatewayCommand() *cobra.Command {
	...
	updateCmd.Flags().StringP("name", "n", "", "NAT gateway name")
	updateCmd.Flags().StringP("resource-group", "g", "", "Resource group name")
	updateCmd.Flags().Int32("idle-timeout", 4, "Idle timeout in minutes (4-120)")
	...
	waitCmd := &cobra.Command{
		Use:   "wait",
		Short: "Wait for a NAT gateway to reach a condition",
		RunE: func(cmd *cobra.Command, args []string) error {
			name, _ := cmd.Flags().GetString("name")
			resourceGroup, _ := cmd.Flags().GetString("resource-group")
	...
	waitCmd.Flags().StringP("name", "n", "", "NAT gateway name")
	waitCmd.Flags().StringP("resource-group", "g", "", "Resource group name")
	waitCmd.Flags().Bool("deleted", false, "Wait until the NAT gateway is deleted")
	waitCmd.Flags().Bool("exists", false, "Wait until the NAT gateway exists")
