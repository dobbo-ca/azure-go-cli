...
// waitDone reports whether polling should stop. found is whether the public IP
// prefix currently exists; state is its last-seen provisioning state. The default
// (and --exists) wait for the terminal "Succeeded" state.
...
func Wait(ctx context.Context, cmd *cobra.Command, name, resourceGroup string, deleted, exists bool, interval, timeout int) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timed out waiting for public IP prefix '%s' after %d seconds", name, timeout)
		}
