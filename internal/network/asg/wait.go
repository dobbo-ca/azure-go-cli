...
// security group currently exists; state is its last-seen provisioning state. The
// default (and --exists) wait for the terminal "Succeeded" state.
	...
	for {
		...
		if err != nil {
			...
			if errors.As(err, &respErr) && respErr.StatusCode == 404 {
				...
				return fmt.Errorf("failed to get application security group: %w", err)
			}
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timed out waiting for application security group '%s' after %d seconds", name, timeout)
		}
