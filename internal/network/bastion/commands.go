...
func NewBastionCommand() *cobra.Command {
  ...
  tunnelCmd.Flags().Int("conn-read-buffer", 8*1024, "Connection-level read buffer size in bytes (default 8KB)")
  tunnelCmd.Flags().Int("conn-write-buffer", 8*1024, "Connection-level write buffer size in bytes (default 8KB)")
  tunnelCmd.Flags().Int("chunk-read-buffer", 4*1024, "Streaming chunk read buffer size in bytes (default 4KB)")
  tunnelCmd.Flags().Int("chunk-write-buffer", 4*1024, "Streaming chunk write buffer size in bytes (default 4KB)")
...
For AAD authentication, provide your Azure AD username (typically your email or UPN).`,
  ...
  sshCmd.Flags().Int("conn-read-buffer", 8*1024, "Connection-level read buffer size in bytes (default 8KB)")
  sshCmd.Flags().Int("conn-write-buffer", 8*1024, "Connection-level write buffer size in bytes (default 8KB)")
  sshCmd.Flags().Int("chunk-read-buffer", 4*1024, "Streaming chunk read buffer size in bytes (default 4KB)")
  sshCmd.Flags().Int("chunk-write-buffer", 4*1024, "Streaming chunk write buffer size in bytes (default 4KB)")
