...
func Create(ctx context.Context, cmd *cobra.Command, resourceGroup, account, name string, quota int32, accessTier string, metadata map[string]string) error {
	...
	if quota > 0 {
		fs.FileShareProperties.ShareQuota = to.Ptr(quota)
	}
	if accessTier != "" {
	...
	resp, err := client.Create(ctx, resourceGroup, account, name, fs, nil)
	if err != nil {
