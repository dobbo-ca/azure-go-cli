...
// currently exists; state is its last-seen ProvisioningState. The default (and
// --created) wait for the terminal "Succeeded" state.
...
func Wait(ctx context.Context, cmd *cobra.Command, name, resourceGroup string, deleted, exists bool, interval, timeout int) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timed out waiting for cluster '%s' after %d seconds", name, timeout)
		}
