...
func Delete(ctx context.Context, cmd *cobra.Command, resourceGroup, vmssName, name string, noWait bool) error {
	...
	fmt.Printf("Deleting extension '%s' from VMSS '%s'...\n", name, vmssName)
	poller, err := client.BeginDelete(ctx, resourceGroup, vmssName, name, nil)
	if err != nil {
