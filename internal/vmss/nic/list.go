...
func List(ctx context.Context, cmd *cobra.Command, resourceGroup, vmssName string) error {
	...
	pager := client.NewListVirtualMachineScaleSetNetworkInterfacesPager(resourceGroup, vmssName, nil)
	for pager.More() {
		page, err := pager.NextPage(ctx)
		if err != nil {
			return fmt.Errorf("failed to list VMSS network interfaces: %w", err)
		}
