...
func List(ctx context.Context, cmd *cobra.Command, resourceGroup, vmssName, instanceID string) error {
	...
	pager := client.NewListPager(resourceGroup, vmssName, instanceID, nil)
	for pager.More() {
		page, err := pager.NextPage(ctx)
		if err != nil {
			return fmt.Errorf("failed to list VMSS VM run commands: %w", err)
		}
