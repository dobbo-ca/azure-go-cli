...
// waitDone reports whether polling should stop. found is whether the scale set
...
func Wait(ctx context.Context, cmd *cobra.Command, name, resourceGroup string, deleted, exists bool, interval, timeout int) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timed out waiting for virtual machine '%s' after %d seconds", name, timeout)
		}
