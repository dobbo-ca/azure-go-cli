...
func Assign(ctx context.Context, cmd *cobra.Command, resourceGroup, name string, identityIDs []string, noWait bool) error {
	...
	if len(identityIDs) > 0 {
		...
		identity.Type = to.Ptr(armcompute.ResourceIdentityTypeUserAssigned)
		identity.UserAssignedIdentities = userAssigned
	} else {
		identity.Type = to.Ptr(armcompute.ResourceIdentityTypeSystemAssigned)
	}

	fmt.Printf("Assigning identity to '%s'...\n", name)
