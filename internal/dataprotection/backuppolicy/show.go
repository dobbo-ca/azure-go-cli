...
func newShowCommand() *cobra.Command {
  cmd := &cobra.Command{
    ...
    Long:  "Gets details of a backup policy",
    RunE: func(cmd *cobra.Command, args []string) error {
      resourceGroup, _ := cmd.Flags().GetString("resource-group")
      vaultName, _ := cmd.Flags().GetString("vault-name")
      name, _ := cmd.Flags().GetString("name")
      return ShowBackupPolicy(context.Background(), cmd, resourceGroup, vaultName, name)
    },
  }
...
func ShowBackupPolicy(ctx context.Context, cmd *cobra.Command, resourceGroup, vaultName, name string) error {
