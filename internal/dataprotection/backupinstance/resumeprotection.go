...
func newResumeProtectionCommand() *cobra.Command {
  cmd := &cobra.Command{
    ...
    Short: "Resume protection of a backup instance",
    Long:  "Resumes protection of a backup instance in a backup vault",
...
func ResumeProtection(ctx context.Context, resourceGroup, vaultName, name string, noWait bool) error {
  ...
  if noWait {
    fmt.Println(`{"status": "Resume-protection operation started. Use 'az dataprotection backup-instance show' to check status."}`)
    return nil
  }

  _, err = poller.PollUntilDone(ctx, nil)
  if err != nil {
    return fmt.Errorf("resume-protection operation failed: %w", err)
  }
