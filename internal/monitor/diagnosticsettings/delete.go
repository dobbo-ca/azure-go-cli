...
func Delete(ctx context.Context, cmd *cobra.Command, resource, name string) error {
	...
	if _, err := client.Delete(ctx, resource, name, nil); err != nil {
		return fmt.Errorf("failed to delete diagnostic setting: %w", err)
	}
