...
// waitDone reports whether polling should stop. found is whether the application
// currently exists; state is its last-seen provisioning state. The default
// (and --created) wait for the terminal "Succeeded" state.
...
func Wait(ctx context.Context, cmd *cobra.Command, resourceGroup, clusterName, applicationName string, deleted, exists bool, interval, timeout int) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timed out waiting for application '%s' after %d seconds", applicationName, timeout)
		}
