...
// waitDone reports whether polling should stop. found is whether the cluster
...
func Wait(ctx context.Context, cmd *cobra.Command, resourceGroup, clusterName string, deleted, exists bool, interval, timeout int) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timed out waiting for cluster '%s' after %d seconds", clusterName, timeout)
		}
