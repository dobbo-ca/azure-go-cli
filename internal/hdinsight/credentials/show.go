...
func Show(ctx context.Context, cmd *cobra.Command, resourceGroup, clusterName string) error {
	...
	resp, err := client.GetGatewaySettings(ctx, resourceGroup, clusterName, nil)
	if err != nil {
		return fmt.Errorf("failed to get gateway settings: %w", err)
	}

	return output.PrintJSON(cmd, resp.GatewaySettings)
}
