...
func TestPrintJSON_HonorsOutputFlag(t *testing.T) {
	...
	t.Run("default", func(t *testing.T) {
		...
		want, _ := marshalIndentNoEscape(data)
...
// TestJSONOutputDoesNotHTMLEscape pins that -o json output leaves &, <, >
// unescaped, matching azure-cli's json.dumps(..., ensure_ascii=False) (knack
// format_json, knack/output.py:33-37) rather than Go's json.MarshalIndent
// default of escaping them to & < >. Reference, verified
// against the knack venv:
//
//	python -c "import json; print(repr(json.dumps({'sig': 'a=1&b=2<c>3'}, ensure_ascii=False, indent=2, sort_keys=True)))"
//	'{\n  "sig": "a=1&b=2<c>3"\n}'
func TestJSONOutputDoesNotHTMLEscape(t *testing.T) {
	type data struct {
		Sig string `json:"sig"`
	}
	d := data{Sig: "a=1&b=2<c>3"}
	const rawSig = "a=1&b=2<c>3"

	t.Run("PrintJSON no query", func(t *testing.T) {
		cmd := &cobra.Command{}
		cmd.Flags().String("query", "", "")
		var buf bytes.Buffer
		cmd.SetOut(&buf)
		if err := PrintJSON(cmd, d); err != nil {
			t.Fatalf("PrintJSON: %v", err)
		}
		got := buf.String()
		if !strings.Contains(got, rawSig) {
			t.Errorf("output does not contain raw sig, got: %q", got)
		}
		if strings.Contains(got, "\\u0026") || strings.Contains(got, "\\u003c") || strings.Contains(got, "\\u003e") {
			t.Errorf("output HTML-escaped &/</>, got: %q", got)
		}
	})

	t.Run("PrintJSON with query", func(t *testing.T) {
		cmd := &cobra.Command{}
		cmd.Flags().String("query", "sig", "")
		var buf bytes.Buffer
		cmd.SetOut(&buf)
		if err := PrintJSON(cmd, d); err != nil {
			t.Fatalf("PrintJSON: %v", err)
		...
		got := buf.String()
		if !strings.Contains(got, rawSig) {
			t.Errorf("queried output does not contain raw sig, got: %q", got)
		}
		if strings.Contains(got, "\\u0026") || strings.Contains(got, "\\u003c") || strings.Contains(got, "\\u003e") {
			t.Errorf("queried output HTML-escaped &/</>, got: %q", got)
		}
	})

	t.Run("PrintFormatted json", func(t *testing.T) {
		cmd := &cobra.Command{}
		cmd.Flags().String("query", "", "")
		var buf bytes.Buffer
		cmd.SetOut(&buf)
		if err := PrintFormatted(cmd, d, "json"); err != nil {
			t.Fatalf("PrintFormatted: %v", err)
		}
		got := buf.String()
		if !strings.Contains(got, rawSig) {
...
// numFidelityDoc carries every numeric shape a float64 round-trip destroys:
// an integer above 2^53 (big) and below -2^53 (small), int64 max
// (items[0].big), a genuine integral float (f, items[1].big), negative zero
// (neg), and a wire literal in exponent form (e).
const numFidelityDoc = `{"big":9007199254740993,"small":-9007199254740993,"f":3.0,"e":1e+16,` +
	`"neg":-0.0,"p":80,"a":[1,2,3],` +
	`"items":[{"n":"x","v":2,"big":9223372036854775807},{"n":"y","v":1,"big":3.0}]}`

func numFidelityData(t *testing.T) interface{} {
	t.Helper()
	var v interface{}
	dec := json.NewDecoder(strings.NewReader(numFidelityDoc))
	dec.UseNumber()
	if err := dec.Decode(&v); err != nil {
...
func printQueried(t *testing.T, data interface{}, queryStr, format string) string {
	t.Helper()
	cmd := &cobra.Command{}
	cmd.Flags().String("query", queryStr, "")
	var buf bytes.Buffer
	cmd.SetOut(&buf)
	if err := PrintFormatted(cmd, data, format); err != nil {
		t.Fatalf("PrintFormatted(%q, %q): %v", queryStr, format, err)
	}
	return buf.String()
}

// TestQueryNumericFidelity pins knack parity for --query results in tsv,
// table and json. Every want below was generated by running knack 0.14.0 +
// jmespath 1.1.0 over numFidelityDoc:
//
//	res = jmespath.compile(q).search(json.loads(DOC), Options(OrderedDict))
//	format_tsv/format_table/format_json(CommandResultItem(res, is_query_active=True))
//
// Before azure-go-cli-v39 these all round-tripped through float64 and
// rendered 9007199254740992 / "3" / 10000000000000000 / "0".
func TestQueryNumericFidelity(t *testing.T) {
	data := numFidelityData(t)
	tests := []struct {
		query   string
		tsv     string
		table   string
		...
		{
			query:   "big",
			tsv:     "9007199254740993\n",
			table:   "Result\n----------------\n9007199254740993\n",
			jsonOut: "9007199254740993\n",
		},
		{
			query:   "small",
			tsv:     "-9007199254740993\n",
		...
		{
			query:   "{B: big, F: f, E: e, N: neg}",
			tsv:     "9007199254740993\t3.0\t1e+16\t-0.0\n",
			table:   "B                 F    E      N\n----------------  ---  -----  ----\n9007199254740993  3.0  1e+16  -0.0\n",
			jsonOut: "{\n  \"B\": 9007199254740993,\n  \"E\": 1e+16,\n  \"F\": 3.0,\n  \"N\": -0.0\n}\n",
		},
		{
			query:   "[big, f, e, neg]",
		...
		{
			query:   "items[0].big",
			tsv:     "9223372036854775807\n",
			table:   "Result\n-------------------\n9223372036854775807\n",
			jsonOut: "9223372036854775807\n",
		},
		{
			query:   "items[].big",
			tsv:     "9223372036854775807\n3.0\n",
		...
		{
			query:   "items[].{Name: n, Big: big}",
			tsv:     "x\t9223372036854775807\ny\t3.0\n",
	...
	for _, tt := range tests {
		t.Run(tt.query, func(t *testing.T) {
			if got := printQueried(t, data, tt.query, "tsv"); got != tt.tsv {
				t.Errorf("tsv = %q, want %q", got, tt.tsv)
			}
			if got := printQueried(t, data, tt.query, "table"); got != tt.table {
				t.Errorf("table = %q, want %q", got, tt.table)
			}
			if got := printQueried(t, data, tt.query, "json"); got != tt.jsonOut {
				t.Errorf("json = %q, want %q", got, tt.jsonOut)
			}
		})
	}
}

// TestQueryNumericBuiltinsUnaffected is the other half of the v39 guard: the
// UseNumber decode must never reach a query containing a function or a
// comparator, because go-jmespath v0.4.0 errors on the former and silently
// returns the wrong rows for the latter. Each want below is what the query
// must still produce; a regression that widened PreservesNumberLiterals would
// turn the function rows into errors and empty the comparator rows.
//
// Every want was generated by running knack 0.14.0 + jmespath 1.1.0 over
// numFidelityDoc, and every one matches EXCEPT avg(a): Python jmespath keeps
// int/float apart in its numeric builtins (sum/max/min/ceil/floor return an
// int for integral input, avg always returns a float), so knack prints
// "2.0\n" where go-jmespath's uniformly-float64 avg gives "2\n". That is a
// go-jmespath return-type divergence, not the v39 input-fidelity one, and is
// out of this fix's scope.
func TestQueryNumericBuiltinsUnaffected(t *testing.T) {
	data := numFidelityData(t)
	tests := []struct {
		query string
		tsv   string
...
// TestQueryNumericFidelityRemainingDivergence documents, so it cannot regress
// unnoticed, the part of azure-go-cli-v39 that is NOT fixed: a query that
// contains a comparator or a function still evaluates over a float64 tree, so
// int64 max still rounds. knack renders "9223372036854775807" here. Closing
// this would require teaching go-jmespath about json.Number, i.e. vendoring
// it (azure-go-cli-xmf).
func TestQueryNumericFidelityRemainingDivergence(t *testing.T) {
	data := numFidelityData(t)
	const want = "9223372036854776000\n" // knack: "9223372036854775807\n"
	if got := printQueried(t, data, "items[?n=='x'].big", "tsv"); got != want {
